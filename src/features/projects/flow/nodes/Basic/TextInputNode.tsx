...
export default memo(({ id }: any) => {
  ...
  const updateDimension = (attr: any) => (event: { target: { value: string; }; }) => {
    ...
    setNodes((nds) =>
      nds.map((n) => {
        if (n.id === '2-3') {
          ...
          const parentWidth: any = parentNode?.style ? parentNode.style.width : Infinity;
          const parentHeight: any = parentNode?.style ? parentNode.style.height : Infinity;
